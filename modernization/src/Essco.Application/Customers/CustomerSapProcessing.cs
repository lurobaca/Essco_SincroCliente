...
public sealed class CustomerSapJobProcessor(ICustomerChangeRepository customers, IGeographyRepository geography, ISapCustomerGateway gateway) : ICustomerSapJobProcessor
{
    ...
    {
        CustomerSapPayload? request; try { request = JsonSerializer.Deserialize<CustomerSapPayload>(payload); } catch (JsonException) { return new(false, null, "Payload de cliente inválido.", false); }
        if (request is null) return new(false, null, "Payload de cliente vacío.", false);
