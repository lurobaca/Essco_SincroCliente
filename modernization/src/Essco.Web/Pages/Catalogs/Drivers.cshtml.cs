...
{
    [BindProperty] public InputModel Input { get; set; } = new(); [BindProperty(SupportsGet = true)] public string? TypeFilter { get; set; }
    public IReadOnlyCollection<Driver> Items { get; private set; } = []; [TempData] public string? StatusMessage { get; set; }
    public static IReadOnlyCollection<string> Types { get; } = ["CHOFER", "AYUDANTE"];
