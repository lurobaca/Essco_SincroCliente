...
public sealed class BackgroundModel(EmployeeService employees, EmployeeBackgroundService service) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(string id, CancellationToken t) { if (!await Load(id, t)) return NotFound(); var today = DateOnly.FromDateTime(DateTime.Today); EducationInput = new() { From = today, To = today }; ExperienceInput = new() { From = today, To = today }; return Page(); }
