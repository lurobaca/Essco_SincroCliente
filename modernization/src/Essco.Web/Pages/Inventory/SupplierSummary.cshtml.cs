...
public sealed class SupplierSummaryModel(InventoryService service, IInventoryConsolidationReadiness readiness) : PageModel
{
    ...
    public IReadOnlyCollection<InventoryGroupCompletion> Groups { get; private set; } = [];
    public bool GroupsCompleted => InventoryConsolidationReadiness.AllCompleted(Groups);
    public async Task<IActionResult> OnGetAsync(int id, CancellationToken token)
    {
        ...
        {
            Groups = await readiness.GetAsync(id, Supplier.Trim(), token);
