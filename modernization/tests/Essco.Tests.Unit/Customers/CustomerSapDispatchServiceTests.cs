...
{
 [Theory][InlineData(CustomerChangeState.New,CustomerSapOperations.Create)][InlineData(CustomerChangeState.Modified,CustomerSapOperations.Update)][InlineData(CustomerChangeState.Close,CustomerSapOperations.Close)]
 public async Task Dispatch_MapsLegacyState(CustomerChangeState state,string expected)
 {var queue=new Queue();var result=await new CustomerSapDispatchService(new Repository(Customer(state)),queue).DispatchAsync(7,"TEST","user",CancellationToken.None);Assert.True(result.Succeeded);Assert.Equal(expected,queue.Request!.OperationType);Assert.Equal("customer-change:7:"+expected,queue.Request.IdempotencyKey);}
