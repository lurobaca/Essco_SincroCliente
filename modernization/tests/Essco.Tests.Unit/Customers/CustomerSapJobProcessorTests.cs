...
public sealed class CustomerSapJobProcessorTests
{
    ...
    { var repository = new Repository(Customer()); var result = await new CustomerSapJobProcessor(repository, new Geography(), new Gateway(new(true, "C1", null, false))).ProcessAsync(CustomerSapOperations.Create, Payload(Customer()), CancellationToken.None); Assert.True(result.Succeeded); Assert.True(repository.ApproveCalled); }
    [Fact]
    public async Task FailedSapResult_DoesNotApproveSqlRequest()
    { var repository = new Repository(Customer()); var result = await new CustomerSapJobProcessor(repository, new Geography(), new Gateway(new(false, null, "SAP error", true))).ProcessAsync(CustomerSapOperations.Create, Payload(Customer()), CancellationToken.None); Assert.False(result.Succeeded); Assert.False(repository.ApproveCalled); }
    [Fact]
    public async Task MismatchedOperation_IsRejectedBeforeSap()
    { var gateway = new Gateway(new(true, "C1", null, false)); var result = await new CustomerSapJobProcessor(new Repository(Customer()), new Geography(), gateway).ProcessAsync(CustomerSapOperations.Close, Payload(Customer()), CancellationToken.None); Assert.False(result.Succeeded); Assert.False(gateway.Called); }
    private static CustomerChangeRequest Customer() => new() { Id = 1, Sequence = "1", Code = "C1", Name = "Cliente", TaxId = "3101123456", IdentificationType = 2, ProvinceId = 1, CantonId = 1, DistrictId = 1, NeighborhoodId = 1, RequestedAt = new DateTime(2026, 1, 1), State = CustomerChangeState.New };
    ...
    public async Task MissingGeography_DoesNotSendNumericNamesToSap()
    {
        ...
        var result = await new CustomerSapJobProcessor(repository, new Geography(), gateway)
            .ProcessAsync(CustomerSapOperations.Create, Payload(Customer() with { ProvinceId = 99 }), default);
        Assert.False(result.Succeeded);
        Assert.False(result.Retryable);
        Assert.False(gateway.Called);
    ...
    public async Task Close_DoesNotRequireGeographyOrTaxData()
    {
        ...
        var result = await new CustomerSapJobProcessor(repository, new UnavailableGeography(), gateway)
            .ProcessAsync(CustomerSapOperations.Close, Payload(Customer() with { State = CustomerChangeState.Close, ProvinceId = 99, TaxId = "" }), default);
    ...
    public async Task InvalidCustomer_DoesNotCallSap()
    {
        ...
        var result = await new CustomerSapJobProcessor(new Repository(Customer() with { TaxId = "" }), new Geography(), gateway)
            .ProcessAsync(CustomerSapOperations.Create, Payload(Customer() with { TaxId = "" }), default);
        Assert.False(result.Succeeded);
        Assert.False(gateway.Called);
    }
    ...
    public async Task ApprovedCustomer_DoesNotHideOperationMismatch()
    {
        ...
        var result = await new CustomerSapJobProcessor(new Repository(Customer() with { Approved = true }), new Geography(), gateway)
            .ProcessAsync(CustomerSapOperations.Close, Payload(Customer() with { Approved = true }), default);
        Assert.False(result.Succeeded);
        Assert.False(gateway.Called);
    }

    private static string Payload(CustomerChangeRequest customer) =>
        JsonSerializer.Serialize(new CustomerSapPayload(customer.Id, CustomerRequestFingerprint.Compute(customer)));

    [Fact]
    public async Task ChangedRequest_IsRejectedBeforeSap()
    {
        var gateway = new Gateway(new(true, "C1", null, false));
        var result = await new CustomerSapJobProcessor(new Repository(Customer() with { Name = "Changed" }), new Geography(), gateway)
            .ProcessAsync(CustomerSapOperations.Create, Payload(Customer()), default);
        Assert.False(result.Succeeded);
        Assert.False(result.Retryable);
        Assert.Contains("cambió", result.Error!);
        Assert.False(gateway.Called);
    }

    [Fact]
    public async Task LegacyJob_RequiresResubmission()
    {
        var gateway = new Gateway(new(true, "C1", null, false));
        var result = await new CustomerSapJobProcessor(new Repository(Customer()), new Geography(), gateway)
            .ProcessAsync(CustomerSapOperations.Create, JsonSerializer.Serialize(new CustomerSapPayload(1)), default);
        Assert.False(result.Succeeded);
        Assert.Contains("versión verificable", result.Error!);
        Assert.False(gateway.Called);
    }

    [Fact]
    public void Fingerprint_IgnoresApprovalButDetectsDataChanges()
    {
        Assert.Equal(CustomerRequestFingerprint.Compute(Customer()), CustomerRequestFingerprint.Compute(Customer() with { Approved = true }));
        Assert.NotEqual(CustomerRequestFingerprint.Compute(Customer()), CustomerRequestFingerprint.Compute(Customer() with { WebPassword = "changed-test-value" }));
    }
