...
Public Class Firma
    ...
    Public Sub FirmaXML_Xades(pathXML As String, certificado As String, esThumbprint As Boolean, clave As String)
        Try
            ...
            parametros.DataFormat = New DataFormat() 'SOLO SIRVE CON LA DLL, D:\FACTURA ELECTRONICA\CODIGOS FUENTE\FacturaElectronicaCR-master\COMPONENTES\Librerias\FirmaXadesNet.dll V.2.1.0 
            parametros.Signer = New FirmaXadesNet.Crypto.Signer(cert)

            Dim fs As FileStream = New FileStream(pathXML & "_SF.xml", FileMode.Open)
            Dim docFirmado As FirmaXadesNet.Signature.SignatureDocument = xadesService.Sign(fs, parametros)
            docFirmado.Save(pathXML & "_Firmado.xml")
        ...
        Catch ex As Exception
            MessageBox.Show("ERROR en FirmaXML_Xades [ " & ex.Message & " : " & pathXML & "] ")
            Throw
        End Try
    End Sub
